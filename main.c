...
void display_sprites(int l, void *p)
{
    ...
    if (sprmode >= 0)
    {
        ...
        {
            for (i = 0; i < 5; i++)
                draw_sprite(i * 64 + 16, j * 64 + 16, 32, 32, 64, test32x32_trans_smileData, DRAWSPR_OVERWRITE | mode);
        }
        draw_pivot_stretch_sprite(160, 112, 32, 32, 64, test32x32_trans_smileData, DRAWSPR_SCALE | mode, 0x10000 + ((start * 16) & 0xffff));
    }
}
